...
export function paginateRows(allRows, offset, limit) {
...
server.tool('append_rows', 'Append rows to a Google Sheet. Values is a 2D array of rows. Range should be like "Sheet1!A:D".', {
    sheet_id: z.string(),
    range: z.string(),
...
server.tool('update_range', 'Overwrite a range in a Google Sheet with the given values (2D array).', {
    sheet_id: z.string(),
    range: z.string(),
    values: z.array(z.array(z.union([z.string(), z.number(), z.boolean()]))),
}, async ({ sheet_id, range, values }) => {
