...
async function spawnContainer(session: Session): Promise<void> {
  ...
  // OneCLI validates identifiers as `[a-z][a-z0-9-]{0,49}` — must start with
  // a letter. Raw agent_group UUIDs sometimes start with a digit (e.g.
  // `7d534d6a-…`), which OneCLI rejects with 400. Derive from the folder
  // slug instead; it's stable, lower-case, and reversible via folder lookup.
  const agentIdentifier = agentGroup.folder.replace(/_/g, '-').toLowerCase();
