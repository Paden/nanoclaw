...
export const COMPACT_TOKEN_THRESHOLD = parseInt(
  ...
  10,
); // Trigger compaction when a session's peak per-API-call input tokens exceeds this
...
// Webhook personas for non-Claudio voices.
// When an agent sends a message with sender matching a key here,
// the message is sent via Discord webhook with that persona's display name.
export const WEBHOOK_PERSONAS: Record<
  string,
